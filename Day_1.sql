...
SELECT product_name from (
	SELECT
		  pa.product_id,
 		  ...
 		  pa.price,
  		  s.country,
 		  RANK() OVER (	PARTITION BY s.country ORDER BY pa.price DESC ) AS price_rank
FROM products AS pa
JOIN suppliers AS s
ON pa.supplier_id = s.supplier_id ) AS ranked_product
WHERE price_rank = 1
...
-- 1.Write an SQL query to calculate the total transaction amount for each customer for the current year. 
-- The output should contain Customer_Name and the total amount.
-- 2. find total transaction amt group by each customer filter with current year 
